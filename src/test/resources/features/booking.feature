Feature: Hotel Reservation System
  ...
  Scenario: Retrieve booking details by ID
    Given the user connects to the API
    And a booking with ID '2' exists
    When the user sends a GET request to '/booking/2'
    Then the response status code should be 200
  ...
  Scenario: Update an existing booking
    ...
    When the user sends a PUT request to '/booking/2' with updated booking data
    Then the response status code should be 200
  ...
  Scenario: Partially update an existing booking
    ...
    And a booking with ID '2' exists
    And the user has a valid authentication token
    When the user sends a PATCH request to '/booking/2' with partial booking data
    Then the response status code should be 200
  ...
  Scenario: Delete an existing booking
    ...
    And a booking with ID '2' exists
    And the user has a valid authentication token
    When the user sends a DELETE request to '/booking/2'
